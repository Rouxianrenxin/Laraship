...
use Corals\Modules\Payment\PayPal\Support\InstantUpdateApi\ShippingOption;
use Corals\Modules\Payment\PayPal\Support\InstantUpdateApi\BillingAgreement;
        ...
        if (!empty($callback)) {
            ...
            if (empty($shippingOptions)) {
                throw new InvalidRequestException(
                    trans('PayPal::exception.invalid_callback_update_api')
                );
            } else {
                $hasDefault = false;
                ...
                if (!$hasDefault) {
                    throw new InvalidRequestException(
                        trans('PayPal::exception.shipping_must_set_default')
                    );
                }
            }
        }
