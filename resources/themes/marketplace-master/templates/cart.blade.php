    ...
    <div class="container padding-bottom-3x mb-1 mt-30">
        @if (count(ShoppingCart::getAllInstanceItems()) > 0)
            <div class="table-responsive shopping-cart">
                <table class="table color-table info-table table table-hover table-striped table-condensed">
                    <thead>
                    <tr>
                        <th>@lang('corals-marketplace-master::labels.template.cart.product')</th>
                        <th class="text-center"
                            width="200">@lang('corals-marketplace-master::labels.template.cart.quantity')</th>
                        <th class="text-center">@lang('corals-marketplace-master::labels.template.cart.price')</th>
                        <th class="text-center">
                            <a class="btn btn-sm btn-outline-danger" href="{{ url('cart/empty') }}"
                               title="Delete" data-action="post" data-page_action="site_reload">
                                @lang('corals-marketplace-master::labels.template.cart.clear_cart')
                            </a>
                        </th>
                    </tr>
                    </thead>
            ...
            <div class="shopping-cart-footer">
                <div class="column">
                    <div class="column"><a class="btn btn-outline-secondary" href="{{ url('shop') }}"><i
                                    class="icon-arrow-left"></i>&nbsp; @lang('corals-marketplace-master::labels.template.cart.back_shopping')
                        </a></div>
                </div>
                <div class="column text-lg">@lang('corals-marketplace-master::labels.template.cart.subtotal')
        ...
        @else
            <div class="text-center">
                <h3>@lang('corals-marketplace-master::labels.template.cart.have_no_item')</h3>
                <a href="{{ url('shop') }}" class="btn btn-primary btn">
                    <i class="fa fa-shopping-cart fa-fw"></i>@lang('corals-marketplace-master::labels.template.cart.continue_shopping')
                </a>
            </div>
        @endif

        @include('partials.featured_products',['title'=>trans('corals-marketplace-master::labels.template.cart.title')])
    </div>
@stop
