@isset($product)
                        ...
                        @if(user() && user()->subscribed(null, $plan->id))
                            <a href="#"
                               class="btn btn-rounded {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}">
                                @lang('corals-marketplace-master::labels.pricing.current_package')
                            </a>
                            <br/>
                        ...
                        @else
                            <a class="btn btn-rounded {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}"
                               href="{{ url('subscriptions/checkout/'.$plan->hashed_id) }}">
                                @lang('corals-marketplace-master::labels.pricing.subscribe_now')
                            </a>
                        @endif
                    </div>
                </div>
    ...
    @else
        <p class="text-center text-danger">
            <strong>@lang('corals-marketplace-master::labels.pricing.product_not_found')</strong></p>
        @endisset