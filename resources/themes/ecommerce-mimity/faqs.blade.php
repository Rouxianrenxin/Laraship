...
@section('editable_content')
    ...
    <div class="container padding-bottom-3x padding-top-3x mb-1 text-center">
        <div class="row mb-5">
            <div class="col-md-12">
                <h4>{!! $faq->title !!}</h4>
                {!! $faq->content !!}
                            ...
                            <div class="col-lg-3 col-md-4">
                                <ul class="list-group nav nav-tabs" role="tablist">
                                    @foreach($categories as $category)
                                        <li class="nav-item">
                                            <a class="list-group-item w-100 {{$loop->first ? 'active' : ''}}"
                                            ...
                                            <div class="accordion" id="accordion{{ $category->id }}" role="tablist">
                                                @if(count($faqs = $category->faqs()->published()->get()))
                                                    @foreach($faqs as $faq)
                                                        <div class="card">
                                                            ...
                                                            <div class="collapse text-left" id="collapse{{ $faq->id }}{{ $i }}" data-parent="#accordion{{ $category->id }}" role="tabpanel">
                                                                <div class="card-body">
                                                                    <p>{!! $faq->content !!}</p>
                                                ...
                                                @else
                                                    <div class="alert alert-warning">
                                                        <h4>@lang('corals-ecommerce-mimity::labels.faqs.no_faqs_found')</h4>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
