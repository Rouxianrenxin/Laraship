...
@isset($product)
                        ...
                        <div class="pricing-header">
                            @if($plan->recommended)
                                <h4 class="price-lable text-white bg-warning">
                                    @lang('corals-quantum-admin::labels.pricing.recommended')
                                </h4>
                            @endif
                                ...
                                @if(user() && user()->subscribed(null, $plan->id))
                                    @if($ends_at = user()->currentSubscription(null, $plan->id)->ends_at)
                                        <span class="text-danger" style="display: block;">
                                            @lang('corals-quantum-admin::labels.pricing.ends_at',['ends_at'=>format_date_time($ends_at)])
                                        </span>
                                    @endif
                                    <a href="#" class="btn btn-info waves-effect waves-light m-t-20">
                                        @lang('corals-quantum-admin::labels.pricing.current_package')
                                    </a>
                                @else
                                    <a class="btn {{ $plan->recommended?'btn-lg btn-info':'btn-success' }} waves-effect waves-light m-t-20"
                                       href="{{ url('subscriptions/checkout/'.$plan->hashed_id) }}">
                                        @lang('corals-quantum-admin::labels.pricing.subscribe_now')
                                    </a>
                                @endif
                            </div>
                        </div>
    ...
    @else
        <p class="text-center text-danger">
            <strong>@lang('corals-quantum-admin::labels.pricing.product_can_not_found')</strong></p>
        @endisset