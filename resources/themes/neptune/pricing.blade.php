...
@isset($product)
                        ...
                        @if(user() && user()->subscribed(null, $plan->id))
                            <a href="#"
                               class="btn btn-rounded {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}">
                                @lang('corals-neptune::labels.current_package')
                            </a>
                            <br/>
                        ...
                        @else
                            <a class="btn btn-rounded {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}"
                               href="{{ url('subscriptions/checkout/'.$plan->hashed_id) }}">
                                @lang('corals-neptune::labels.subscribe_now')
                            </a>
                        @endif
                    </div>
                </div>
    ...
    @else
        <p class="text-center text-danger"><strong> @lang('corals-neptune::labels.product_cannot_found')</strong></p>
        @endisset